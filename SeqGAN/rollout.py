...
class ROLLOUT(object):
    ...
    def create_gate(self, x, w, u, b, phs):
        # New gate
        gate = tf.sigmoid(
                tf.matmul(x, w) +
                tf.matmul(phs, u) + b
    ...
    def create_memory_cell(self, x, w, u, b, phs):
        # New Memory Cell
        cell = tf.nn.tanh(
                tf.matmul(x, w) +
                tf.matmul(phs, u) + b
    ...
    def create_recurrent_unit(self):
        ...
        def unit(x, hidden_memory_tm1):
            ...
            i = self.create_gate(x, self.Wi, self.Ui, self.bi, previous_hidden_state) #input gate
            f = self.create_gate(x, self.Wf, self.Uf, self.bf, previous_hidden_state) #forget gate
            o = self.create_gate(x, self.Wog, self.Uog, self.bog, previous_hidden_state) #output gate
            c_ = self.create_memory_cell(x, self.Wc, self.Uc, self.bc, previous_hidden_state) #memory cell
